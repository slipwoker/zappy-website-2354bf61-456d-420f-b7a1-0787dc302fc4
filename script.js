...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (!window.zappyContactFormLoaded) {
        console.log('⚠️ Zappy: Skipping duplicate API call');
      } else {
        (async function() {
          try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
              ...
              body: JSON.stringify({
                websiteId: '2354bf61-456d-420f-b7a1-0787dc302fc4',
                name: formData.name,
            ...
            if (response.ok) {
              ...
              console.error('❌ Zappy: Failed to send email - Server responded with status:', response.status);
            }
          } catch (error) {
            console.error('❌ Zappy: Failed to send email', error);
            // Don't break existing functionality - continue with form submission
          }
        })();
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מאגי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
